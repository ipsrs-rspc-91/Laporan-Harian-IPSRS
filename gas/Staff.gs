...
function getStaffMonitoring(session, bulan, tanggal) {
  ...
  /*
   * Pastikan tanggal berada pada bulan yang dipilih.
   */
  if (tanggal.indexOf(bulan) !== 0) {
    tanggal =
      bulan === todayStr.substring(0, 7)
        ? todayStr
        : bulan + '-01';
  }
  ...
  /*
   * Baca seluruh laporan bulan yang dipilih sekali.
   */
  for (
    let i = 1;
    i < reportRows.length;
    ...
    if (!r.staff_id || !r.tanggal) {
      continue;
    }

    const tgl = r.tanggal.toString();

    if (tgl.indexOf(bulan) !== 0) {
      continue;
    }

    byStaffMonthTotal[r.staff_id] =
      (byStaffMonthTotal[r.staff_id] || 0) + 1;

    byStaffDate[r.staff_id] =
      byStaffDate[r.staff_id] || {};

    byStaffDate[r.staff_id][tgl] =
      (byStaffDate[r.staff_id][tgl] || 0) + 1;

    if (tgl === tanggal) {
      byStaffTodayCount[r.staff_id] =
        (byStaffTodayCount[r.staff_id] || 0) + 1;
    }
  }

  const wajibDates =
    getHariWajibDalamBulan_(bulan);

  const bulanBerjalan =
    bulan === todayStr.substring(0, 7);

  const tanggalTidakWajib =
    LIBUR_TANGGAL.indexOf(tanggal) > -1;

  const out = [];

  /*
   * Gabungkan master STAFF dengan transaksi REPORTS.
   */
  for (
    let i = 1;
    i < staffRows.length;
    ...
    out.push({
      ...
      laporan_hari_ini:
        byStaffTodayCount[s.staff_id] || 0,

      sudah_isi_hari_bulan_ini:
        sudahIsiHari,

      belum_isi_hari_bulan_ini:
        belumIsiHari,

      total_transaksi_bulan_ini:
        byStaffMonthTotal[s.staff_id] || 0
    });
  }
...
/**
 * Hitung daftar tanggal wajib lapor dalam satu bulan.
 */
 ...
 * - daftar laporan bulan tersebut
 */
function getStaffDailyStatus(
  session,
  staffId,
  bulan
) {
  ...
  if (!s) {
    return {
      ok: false,
      msg: 'Staf tidak ditemukan.'
    };
  }

  const reportSheet =
    getSheet(SHEET_REPORTS);
...
function getMonthlyRecap(
  ...
  perStaff.forEach(
    function(s) {
      ...
      perBidangMap[key].hari_isi +=
        s.sudah_isi_hari_bulan_ini;

      perBidangMap[key].total_transaksi +=
        s.total_transaksi_bulan_ini;
    }
  );
  ...
  const totalHariWajib =
    monRes.hari_wajib_terhitung;

  const totalStaffDaysWajib =
    totalStaff *
    totalHariWajib;

  const totalStaffDaysSudahIsi =
    perStaff.reduce(
      function(sum, s) {
        return (
          sum +
  ...
  const totalStaffDaysBelumIsi =
    Math.max(
      0,
      totalStaffDaysWajib -
      totalStaffDaysSudahIsi
    );
  ...
  return {
    ...
    per_bidang:
      Object.keys(perBidangMap)
        .map(function(k) {
          return perBidangMap[k];
        })
        .sort(
          function(a, b) {
            return a.bidang < b.bidang
              ? -1
              : 1;
          }
        ),
    ...
    ringkasan: {
      ...
      total_staff_days_wajib:
        totalStaffDaysWajib,

      total_staff_days_sudah_isi:
        totalStaffDaysSudahIsi,

      total_staff_days_belum_isi:
        totalStaffDaysBelumIsi,
...
/**
 * Daftar laporan seorang staf pada bulan tertentu.
 */
 ...
 * - area kerja
 *
 * Tidak digunakan untuk otomatis menentukan
 * siapa yang memiliki kinerja terbaik/terburuk.
 */
function getStaffPerformance(
  ...
  for (
    ...
    if (r.kategori) {
      byKategori[r.kategori] =
        (byKategori[r.kategori] || 0) + 1;
    }

    if (r.area_kerja) {
      byArea[r.area_kerja] =
        (byArea[r.area_kerja] || 0) + 1;
    }
  }
  ...
  return {
    ...
    aktivitas: {
      ...
      hari_isi:
        Object.keys(tanggalSet).length,

      kategori:
        toSortedArray(byKategori),

      area_kerja:
        toSortedArray(byArea)
    },
 ...
 * LIST STAFF
 *
 * Digunakan untuk dropdown/filter Petugas.
 *
 * Tidak mengembalikan field shift.
 */
function listStaff(session) {

  const staffSheet =
    getSheet(SHEET_STAFF);
