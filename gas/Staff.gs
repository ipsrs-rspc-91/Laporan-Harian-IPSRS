...
function getStaffDailyStatus(session, staffId, bulan) {
  ...
  laporanList.sort(function(a, b) { return a.ID < b.ID ? 1 : -1; });
  ...
  const lastDayOfMonth = new Date(year, month, 0).getDate();
  const kalender = [];
  const isAktif = (s.status || 'Aktif').toString().trim().toLowerCase() === 'aktif';
...
function getMonthlyRecap(session, bulan) {
  ...
  return {
    ...
    bulan: bulan,
    per_staff: perStaff,
    per_bidang: Object.keys(perBidangMap).map(function(k) { return perBidangMap[k]; }).sort(function(a, b) {
...
/**
 * Daftar laporan seorang staf pada bulan berjalan.
 */
